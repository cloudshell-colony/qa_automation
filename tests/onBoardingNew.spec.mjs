...
import { executeCLIcommand, overwriteAndSaveToFile, selectFromDropbox } from "./functions/general.mjs";
...
test.describe.serial('onboarding flow', () => {
    ...
    test('Create execution host deployment file', async () => {
        ...
        console.log("got API session: " + await session);
    ...
    test('complete the actions in the GUI after execution host agent is created in K8S', async () => {
        await page.waitForLoadState();
        // since new namespace is used we need a longer wait time for the agent to deploy
    ...
    test('launch s3 blueprint', async () => {
        ...
        await page.click(`[class~="select-inputs\.host_name"]`);
        await page.type(`[class~="select-inputs\.host_name"]`, executionHostName);
