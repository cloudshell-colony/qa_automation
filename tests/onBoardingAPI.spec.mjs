...
const mail = prefix.concat("@").concat(id).concat(".com");
...
const autoGeneratedRepo = 'qtorque';
const assetName = 's3';
...
test.describe.serial('On boarding with APIs', () => {
    ...
    test('Generate blueprints from asset repo', async() => {
        const assetNameList = [assetName]
        await generateSpecificAssetsFromRepoAPI(session, baseURL, spaceName, repoName, assetNameList)
    });

    test('Publish generated blueprint', async() =>{
        blueprintName = 'autogen_'.concat(assetName)
        const response = await publishBlueprintAPI(session, baseURL, spaceName, blueprintName, autoGeneratedRepo)
        await validateAPIResponseis200(response);
    })
    ...
    test('Launch new sandbox', async () => {
        ...
        const resp = await launchBlueprintAPI(session,  baseURL, blueprintName, spaceName, inputs, autoGeneratedRepo);
        const jsonResponse = await resp.json()
        console.log('the json of SB in OnBoardingAPI is '+ JSON.stringify(jsonResponse));
